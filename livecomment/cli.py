...
def add_announce_args(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument(
        "--prefix",
        default=DEFAULT_ANNOUNCE_PREFIX,
        help=f"Text prepended to every announcement. Default: {DEFAULT_ANNOUNCE_PREFIX}.",
    )
    add_message_guard_args(parser)
...
def cmd_announce(args: argparse.Namespace) -> int:
    ...
    messages = prefix_announce_messages(
        messages,
        prefix=args.prefix,
        max_length=args.max_length,
    )
...
def prefix_announce_messages(
    ...
    *,
    ...
    if not normalized_prefix:
        return messages
    return [
        normalize_message(f"{normalized_prefix} {message}", max_length=max_length)
